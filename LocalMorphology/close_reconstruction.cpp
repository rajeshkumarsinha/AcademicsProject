...
Image erosion_mask_min(Image image,int highpow)
{
    int row=image.getrow();
    int col=image.getcol();
    vii img=image.getimg();
...
Image gl_close(Image image,vii structure,int highpow)
{
    Image dilated=dilation(image,structure,highpow);
    Image eroded=erosion(dilated,structure,highpow);
    Image closed=shift(eroded,structure);
    return closed;
}
...
int main()
{
   ...
   /* vii img=image.getimg();
    ...
    Image closed=gl_close(image,structure,highpow);
    Image copy_img=closed;

    int count=0;
    do
    {
        closed=erosion_mask_min(closed,highpow);
        lower_limit(closed,image);
        swapper(copy_img,closed,count);
    }while(count != 0);

    copy_img.imwrite("close_recons.ppm");
